...
from apps.cart.serializers import CartAddItemSerializer, CartInitializeSerializer, CartSerializer, CheckoutSerializer
...
class CartViewSet(viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        return Cart.objects.filter(customer=self.request.user).select_related("restaurant", "delivery_location", "slot")
    ...
    def create(self, request):
        ...
        cart = CartService.initialize_cart(
            ...
            restaurant=Restaurant.objects.get(pk=serializer.validated_data["restaurant_id"]),
            delivery_location=DeliveryLocation.objects.get(pk=serializer.validated_data["delivery_location_id"]),
            slot=DeliverySlot.objects.get(pk=serializer.validated_data["slot_id"]),
            delivery_date=serializer.validated_data["delivery_date"],
        )
        return response.Response(self.get_serializer(cart).data, status=status.HTTP_201_CREATED)
    ...
    def add_item(self, request, pk=None):
        ...
        serializer.is_valid(raise_exception=True)
        CartService.add_item(cart, serializer.validated_data["menu_item"], serializer.validated_data["quantity"])
    ...
    def checkout(self, request, pk=None):
        ...
        serializer.is_valid(raise_exception=True)
        order = CartService.checkout(cart, serializer.validated_data["payment_method"], request.user)

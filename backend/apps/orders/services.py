...
class OrderStateMachine:
    ...
    def transition(cls, order: Order, target_status: str, actor, note: str = "") -> Order:
        logger.info("order_transition_requested", extra={"order_id": str(order.id), "order_number": order.order_number, "from_status": order.status, "to_status": target_status, "actor_id": str(actor.id), "actor_role": actor.role})
        if actor.role not in {"RESTAURANT_ADMIN", "PLATFORM_ADMIN"}:
            logger.warning("order_transition_rejected_actor_role", extra={"order_id": str(order.id), "actor_id": str(actor.id), "actor_role": actor.role})
        ...
        if actor.role == "RESTAURANT_ADMIN" and order.restaurant.owner_id != actor.id:
            logger.warning("order_transition_rejected_wrong_owner", extra={"order_id": str(order.id), "actor_id": str(actor.id), "restaurant_id": str(order.restaurant_id)})
            raise PermissionDenied("You do not manage this restaurant.")
        if target_status == Order.Status.DELIVERED:
            logger.warning("order_transition_rejected_delivered_without_pin", extra={"order_id": str(order.id), "actor_id": str(actor.id)})
    ...
    def verify_pin(cls, order: Order, pin: str, actor) -> Order:
        logger.info("order_pin_verify_requested", extra={"order_id": str(order.id), "order_number": order.order_number, "status": order.status, "actor_id": str(actor.id)})
        if order.status != Order.Status.REACHED:
            logger.warning("order_pin_verify_rejected_status", extra={"order_id": str(order.id), "status": order.status})
        ...
        if order.delivery_pin_attempts >= 3:
            logger.warning("order_pin_verify_rejected_max_attempts", extra={"order_id": str(order.id), "attempts": order.delivery_pin_attempts})
        ...
        if pin != order.delivery_pin:
            ...
            logger.warning("order_pin_verify_failed", extra={"order_id": str(order.id), "attempts": order.delivery_pin_attempts})
        ...
        if hasattr(order, "payment") and order.payment.method == Payment.Method.COD:
            ...
            logger.info("cod_payment_marked_success", extra={"order_id": str(order.id), "payment_id": str(order.payment.id)})
        logger.info("order_pin_verify_succeeded", extra={"order_id": str(order.id), "order_number": order.order_number, "actor_id": str(actor.id)})
        return order

    @classmethod
    def _validate(cls, from_status: str, to_status: str) -> None:
        if to_status not in cls.valid_transitions.get(from_status, set()):
            logger.warning("order_transition_invalid", extra={"from_status": from_status, "to_status": to_status})
    ...
    def _save_transition(cls, order: Order, target_status: str, actor, note: str) -> Order:
        ...
        logger.info("order_transition_saved", extra={"order_id": str(order.id), "order_number": order.order_number, "from_status": from_status, "to_status": target_status, "actor_id": str(actor.id)})
        return order

...
def order_status_notification_task(order_id: str) -> None:
    ...
    latest_history = order.history.order_by("-created_at").first()
    status_label = order.status.replace("_", " ").title()
    message = f"Your order {order.order_number} is now {order.status}."
    if (
        order.status == Order.Status.CANCELLED
        and latest_history
        and latest_history.note
    ):
        message = f"Your order {order.order_number} was cancelled. Reason: {latest_history.note}"
    ...
    logger.info(
        "order_status_notification_created",
        extra={
